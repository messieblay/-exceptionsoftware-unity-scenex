...
namespace ExceptionSoftware.ExScenes
{
    ...
    public class TableWithModel<T> : TreeView where T : TableElement
    {
        ...
        TableFilter _filter = new TableFilter();
        public TableFilter Filter { get { return _filter; } }
        public void RefreshTable()
        {
            searchString = " " + _filter.textFilter;
            Reload();
        }
        public TableWithModel(TreeViewState state, TreeModel<T> model) : base(state)
        {
            Init(model);
        }

        public TableWithModel(TreeViewState state, MultiColumnHeader multiColumnHeader, TreeModel<T> model) : base(state, multiColumnHeader)
        {
            Init(model);
        }
        ...
        protected override IList<TreeViewItem> BuildRows(TreeViewItem root)
        {
            ...
            m_Rows.Clear();
            //_filter.textFilter = searchString.Trim();
        ...
        protected virtual void SortSearchResult(List<TreeViewItem> rows) => rows.Sort((x, y) => EditorUtility.NaturalCompare(x.displayName, y.displayName)); // sort by displayName by default, can be overriden for multicolumn solutions

        protected override IList<int> GetAncestors(int id) => m_TreeModel.GetAncestors(id);

        protected override IList<int> GetDescendantsThatHaveChildren(int id) => m_TreeModel.GetDescendantsThatHaveChildren(id);
        ...
        protected const string k_GenericDragID = "GenericDragColumnDragging";
